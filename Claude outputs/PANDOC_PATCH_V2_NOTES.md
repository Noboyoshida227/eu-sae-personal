...
| `docs/CHANGELOG.md` | modified | entry for `5.2.0-rc.6-wizard.5.6` (run `python tools\bump_version.py 5.2.0-rc.6-wizard.5.6` before `Release.ps1`). |
...
CHANGELOG heading is `## 5.2.0-rc.6-wizard.5.6`; if you stamp a different label
with `bump_version.py`, rename that heading to match (Release.ps1 and
run_tests.R both check it). Suggested:

```
python tools\bump_version.py 5.2.0-rc.6-wizard.5.6 --release-name EU_SAE_520_w5d
```
